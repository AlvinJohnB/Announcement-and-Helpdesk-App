...
const QCHomeTab = () => {
  ...
  return (
    <div>
                  ...
                  <div className="col-md-4" key={test._id}>
                      ...
                      style={{
                        borderLeft: `8px solid ${
                          test.status === "QC Passed"
                            ? "#198754"
                            : test.status === "QC Troubleshooting"
                            ? "#ffc107"
                            : test.status === "For Send-out"
                            ? "#dc3545"
                    ...
                    >
                      ...
                      <div className="card-body">
                        ...
                        {test.status === "Remaining Test" && (
                          <p className="mb-0">
                            Remaining: <strong>{test.remaining}</strong>
                          </p>
                        )}
                        {test.remarks && (
                          <p className="mb-0 text-muted">
                            <em>Remarks: {test.remarks}</em>
                          </p>
                        )}
                      </div>
                    </div>
      ...
      {tests.length === 0 && (
        <div className="text-muted">No QC tests found.</div>
      )}
    </div>
  );
};
