...
const AnnouncementForm = ({ refreshAnnouncements }) => {
  ...
  // Update formData when user.department changes
  React.useEffect(() => {
    if (user?.department) {
  ...
  const onSubmit = async (e) => {
    ...
    const isDepartmentEmpty = !department;
    ...
    try {
      ...
      setMessage({
        type: "danger",
        text: err.response?.data?.msg || "Error posting announcement",
      });
      console.error(err);
    }
        ...
        <form onSubmit={onSubmit}>
          ...
          >
            {isSubmitting ? (
              ...
              <>
                <i className="fas fa-paper-plane me-2"></i> Post Announcement
              </>
            )}
          </button>
        </form>
